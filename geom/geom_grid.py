...
class StructuredGrid(object):
    ...
    def return_VTK(self):
        """
        returns the x, y, z, data arrays in the VTK data format
        """
        length = self.nk*self.nj*self.ni
        out = zeros((length,3))
        count = 0
        for k in range(self.nk):
            for j in range(self.nj):
                for i in range(self.ni):
                    out[count,0] = self.x[i,j,k]
                    out[count,1] = self.y[i,j,k]
                    out[count,2] = self.z[i,j,k]
                    count += 1
        return out
